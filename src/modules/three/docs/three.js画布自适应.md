...
const notEnglishMarkdownRegExp = /-([a-z]{2})\.md$/ // xxx-pt.md  xxx-zh.md
...
module.exports = async function demoLoader() {
  ...
  await Promise.all(
    demoNames.map(async demoName => {
      ...
      const moduleID = `./${demoName.replace(`pages/${pageFilename.replace(/^\/src\/pages\//, '')}/`, '')}`
      const moduleFilepath = path.join(path.dirname(this.resourcePath), moduleID.replace(/\//g, path.sep))
      ...
      try {
        ...
        const moduleTSFilepath = path.join(path.dirname(this.resourcePath), moduleTS.replace(/\//g, path.sep))
